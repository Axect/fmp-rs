...
with plt.style.context(["science", "nature"]):
    fig, axs = plt.subplots(5, 1, figsize=(6, 8), sharex=True, gridspec_kw={'height_ratios': [4, 1, 1, 1, 1]})
    ...
    # Histogram for MACD
    macd_up = np.tanh(np.array(macd - macd_signal) / np.max(macd_signal) * 10)
    macd_up[macd_up < 0] = np.nan
    macd_down = np.tanh(np.array(macd - macd_signal) / np.max(macd_signal) * 10)
    macd_down[macd_down > 0] = np.nan
    axs[2].set_ylim([-1, 1])
    axs[2].bar(x, macd_up, color='r', width=0.8)
    axs[2].bar(x, macd_down, color='b', width=0.8)
    axs[2].set_ylabel("MACD Osc")
    axs[2].grid(True)
